...
class AGU(nLoopRegs : Int, nConstRegs: Int, nLayers: Int, nMultUnits: Int, nAddUnits: Int) extends Module
{
    ...
    */
    for (i <- 0 until nLayers)
    {
        ...
        {
            MultLayers(i)(j).inA := routing(i).outputs(j)(0)
            MultLayers(i)(j).inB := routing(i).outputs(j)(1)
            for (x <- 2 until 4)
            {
...
object Elaborate extends App {
  (new ChiselStage).execute(
    args,
    Seq(ChiselGeneratorAnnotation(() => new AGU(4, 4, 2, 4, 4)),TargetDirAnnotation("build") // Optional: output directory)
  ))
}